...
import { listCharacterSheets, type SheetListItem } from '@/services/sheetService'
  ...
  useEffect(() => {
    ...
    const loadSheets = async () => {
      try {
        ...
        if (isMounted) {
          setError(err instanceof Error ? err.message : 'Failed to load character sheets')
        }
      } finally {
        if (isMounted) {
          setIsLoading(false)
          ...
          <div className="flex flex-col gap-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-[#3e2a1d]">
                  Consultez toutes les fiches PJ disponibles.
                </p>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Link to="/character/new">
                <Button>Créer une fiche PJ</Button>
              </Link>
            </div>
            ...
            {!isLoading && !error && sheets.length === 0 && (
              <div className="text-sm text-[#3e2a1d]">Aucune fiche PJ disponible.</div>
            )}
            ...
            {!isLoading && !error && sheets.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {sheets.map((sheet) => {
                  ...
                  return (
                    <Link key={sheet.id} to={`/character/${identifier}`} className="block group">
